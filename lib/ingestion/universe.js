...
async function fetchBinanceUniverse() {
  ...
  let candidates = [];
  let stats = { nonUsdtOrInactive: 0, stable: 0, leveraged: 0, lowVolume: 0, notTop100: 0, tooYoung: 0, apiFails: 0 };
  
  // 3. Filter raw tickers
  for (const t of tickers) {
    if (!t.symbol.endsWith('USDT') || !validSymbols.has(t.symbol)) {
      stats.nonUsdtOrInactive++;
      continue;
    }
    ...
    if (stablecoinSet.has(base) || /^(USD|EUR|GBP|JPY|AUD|CAD|CHF|CNY|TRY|BRL|RUB)[0-9]*$/.test(base)) {
      stats.stable++;
      continue;
    }
    
    // Leveraged token filter (UP/DOWN, BULL/BEAR)
    if (/UP$|DOWN$|BULL$|BEAR$|HALF$/.test(base)) {
      stats.leveraged++;
      continue;
    }
  ...
  if (candidates.length > 100) {
    stats.notTop100 = candidates.length - 100;
  ...
  console.log(`[universe] Accounting Summary: Fetched ${tickers.length} tickers. Filtered out: ${stats.nonUsdtOrInactive} non-USDT/inactive, ${stats.stable} stable/fiat/synthetics, ${stats.leveraged} leveraged, ${stats.lowVolume} low volume (<$50M), ${stats.notTop100} not in top 100 vol, ${stats.tooYoung} too young (<1yr), ${stats.apiFails} API fails.`);
