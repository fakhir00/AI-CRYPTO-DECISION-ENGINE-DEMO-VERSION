...
async function fetchBinanceUniverse() {
  ...
  let candidates = [];
  let stats = { stable: 0, leveraged: 0, lowVolume: 0, tooYoung: 0, apiFails: 0 };
  ...
  console.log(`[universe] Accounting Summary: Fetched ${tickers.length} tickers. Filtered out: ${stats.stable} stable/fiat/synthetics, ${stats.leveraged} leveraged, ${stats.lowVolume} low volume (<$10M), ${stats.tooYoung} too young (<1yr), ${stats.apiFails} API fails.`);
