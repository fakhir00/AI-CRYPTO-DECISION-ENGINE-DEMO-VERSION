...
/** Fetch technicals for multiple symbols in parallel chunks. */
export async function fetchTechnicalsBatch(symbols = ['BTC', 'ETH', 'SOL'], interval = '15m') {
  const results = [];
  const chunkSize = 3; // Max 3 symbols per chunk
  
  for (let i = 0; i < symbols.length; i += chunkSize) {
    const chunk = symbols.slice(i, i + chunkSize);
    const chunkResults = await Promise.allSettled(chunk.map(s => fetchTechnicals(s, interval)));
    results.push(...chunkResults.flatMap(r => r.status === 'fulfilled' ? r.value : []));
    
    // Delay between chunks to respect TAAPI rate limits
