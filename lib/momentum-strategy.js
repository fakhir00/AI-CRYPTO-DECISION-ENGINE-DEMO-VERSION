...
export const DEFAULT_MOMENTUM_CONFIG = {
  ...
  entryZoneMinWidthPct: 0.10,
  entryZoneMaxWidthPct: 0.20,
  minTargetPct: 0.5,
  tp1Pct: 0.6,
  tp2Pct: 1.3,
  tp3Pct: 2.3,
...
function detectBreakoutCandidate(candles, indicators, direction, config) {
  ...
  for (let i = last; i >= minIndex; i--) {
    ...
    if (!(volumeRatio >= config.volumeMultiplier)) continue;
...
function appendMetricRejectionReasons(reasons, metrics = {}, config) {
  const minTp1Pct = Number(config.minTargetPct) || DEFAULT_MOMENTUM_CONFIG.minTargetPct;
  ...
  const maxZoneWidthPct = Number(config.entryZoneMaxWidthPct) || DEFAULT_MOMENTUM_CONFIG.entryZoneMaxWidthPct;
  const minVolumeRatio = Number(config.volumeMultiplier) || DEFAULT_MOMENTUM_CONFIG.volumeMultiplier;
  ...
  const minStopPct = Number(config.minStopPct) || DEFAULT_MOMENTUM_CONFIG.minStopPct;
  const epsilon = 0.000001;

  const tp1Pct = Number(metrics.tp1Pct);
  if (Number.isFinite(tp1Pct) && tp1Pct + epsilon < minTp1Pct) {
    pushRejectionReason(reasons, `TP1 <${formatThreshold(minTp1Pct)}%`);
  }
  ...
  if (
    Number.isFinite(zoneWidthPct)
    && (zoneWidthPct + epsilon < minZoneWidthPct || zoneWidthPct - epsilon > maxZoneWidthPct)
  ) {
  ...
  if (Number.isFinite(volumeRatio) && volumeRatio + epsilon < minVolumeRatio) {
    pushRejectionReason(reasons, 'volume low');
  }
  ...
  if (Number.isFinite(stopDistancePct) && stopDistancePct + epsilon < minStopPct) {
    pushRejectionReason(reasons, 'stop too tight');
  }
...
function chooseStructuralStop(direction, avgEntry, setup, config) {
  ...
  const stopOptions = direction === 'LONG'
    ...
    : [
        ...
        { basis: 'BREAKOUT_CANDLE_HIGH', reason: 'above breakout candle', price: setup.candle.high * (1 + buffer) }
      ];
  ...
  if (widestPct < config.minStopPct) {
  ...
  if (valid[0].pct < config.minStopPct) return { status: 'NO_SIGNAL', reason: 'STOP_TOO_TIGHT' };
...
function buildTradePlan(symbol, direction, setup, indicators, config, context) {
  ...
  if (stopChoice.status !== 'OK') {
    ...
    if (stopChoice.reason === 'STOP_TOO_TIGHT') {
      pushRejectionReason(rejectionReasons, 'stop too tight');
    }
  ...
  managedSignal.stopReason = stopChoice.reason;
  managedSignal.volumeConfirmation = {
    ratio: setup.volumeRatio,
    text: `${setup.volumeRatio.toFixed(2)}x avg`
  };
  ...
  return {
    ...
    atrPct: atrPct * 100,
    volumeMultiple: setup.volumeRatio,
