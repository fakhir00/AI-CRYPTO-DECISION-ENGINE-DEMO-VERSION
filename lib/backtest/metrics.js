...
export function computeMetrics(signals = []) {
  ...
  for (const sig of signals) {
    ...
    // Ignore signals rejected during level generation (e.g., risk > max_stop_pct)
    if (o.result === 'no_levels') continue;
    ...
    // Additional categorization first
    if (o.result === 'expired' || o.result?.includes('_expired')) {
      expired++;
      // Still include in gross profit/loss for accurate total R
  ...
  const resolvedTotal = wins + losses + breakevens;
  const winRate = resolvedTotal > 0 ? parseFloat((wins / resolvedTotal * 100).toFixed(1)) : 0;
  
  // 95% Wilson Score Interval for Win Rate
  let winRateCI = [0, 0];
  if (resolvedTotal > 0) {
    const z = 1.96;
    const p = wins / resolvedTotal;
    const n = resolvedTotal;
    const denom = 1 + z * z / n;
  ...
  const avgR = resolvedTotal > 0 ? parseFloat((totalR / resolvedTotal).toFixed(3)) : 0;
  const profitFactor = grossLoss > 0 ? parseFloat((grossProfit / grossLoss).toFixed(2)) : grossProfit > 0 ? Infinity : 0;
  const avgBars = resolvedTotal > 0 ? Math.round(totalBars / resolvedTotal) : 0;

  const validSignals = signals.filter(s => s.outcome?.result !== 'no_levels');
  const rejected = signals.length - validSignals.length;
  ...
  const firstIdx = validSignals[0]?.candleIndex ?? 0;
  const lastIdx = validSignals[validSignals.length - 1]?.candleIndex ?? 0;
