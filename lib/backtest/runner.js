...
import { SCORING_CONFIG as PRODUCTION_CONFIG } from '../config.js';
...
async function fetchHistoricalFNG(days = 2000) { // Increased to 2000 so old data doesn't drop off
  try {
...
export async function fetchAllData(symbol, startTime, endTime) {
  ...
  const cacheKeyBase = `${symbol}_${startTime}_${endTime}`;
  
  const [candles, funding] = await Promise.all([
    getCachedOrFetch(`klines_${cacheKeyBase}`, () => fetchHistoricalKlines(symbol, '15m', startTime, endTime)),
    getCachedOrFetch(`funding_${cacheKeyBase}`, () => fetchHistoricalFunding(symbol, startTime, endTime)),
  ]);
  console.log(`[backtest]   Klines: ${candles.length} candles`);
  
  // FNG is global, cache it separately
  // Note: we can't easily pass it back via this function's return signature cleanly without breaking existing calls,
  // but wait, FNG is currently fetched dynamically inside compare-trades and run-backtest!
  // Ah! `fetchAllData` doesn't fetch FNG!
  
  return { candles, funding };
}

// Export the cached FNG fetcher as well
export async function fetchAllFNG() {
  const fng = await getCachedOrFetch('fng_global', () => fetchHistoricalFNG(2000));
  console.log(`[backtest]   Fear & Greed: ${fng.length} daily records`);
  return fng;
}
...
export function simulateOutcome(candles, entryIdx, signal, cfg = {}) {
