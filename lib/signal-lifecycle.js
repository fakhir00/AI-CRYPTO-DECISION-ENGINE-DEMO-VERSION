...
export function formatManagedSignalText(signal = {}) {
  ...
  const entries = (signal.entryZone?.prices || []).map(price => formatLifecyclePrice(price, reference));
  ...
  const volumeText = signal.volumeConfirmation?.text || (Number.isFinite(Number(signal.volumeMultiple)) ? `${Number(signal.volumeMultiple).toFixed(2)}x avg` : 'N/A');
  const stopReason = signal.stopReason || 'N/A';
  const entryWidth = Number(signal.entryZoneWidthPct ?? signal.entryZone?.widthPct);

  return `#${signal.pair || `${signal.symbol || 'BTC'}/USDT`}

Signal ID: ${signal.signalId || 'UNKNOWN'}
...
Entry Width:
${Number.isFinite(entryWidth) ? `${entryWidth.toFixed(2)}%` : 'N/A'}
