...
const OUTPUT_MIN_VOLUME_RATIO = 0.01;
const OUTPUT_MAX_ENTRY_WIDTH_PCT = 5.0;
const OUTPUT_MIN_TP1_PCT = 0.01;
const OUTPUT_MIN_STOP_DISTANCE_PCT = 0.01;
...
export function normalizeSignalStopReason(reason = '', direction = 'LONG') {
  let clean = String(reason || '').trim().toLowerCase().replace(/\s+/g, ' ');
  const side = String(direction || 'LONG').toUpperCase();
  const isShort = side === 'SHORT' || side === 'SELL';

  if (!clean) {
    return isShort ? 'above swing high' : 'below swing low';
  }

  // Direction-aware auto-correction to ensure Bybit/BingX/OKX Free Text parsers do not get confused
  if (isShort) {
    if (clean.includes('below')) {
      clean = clean.replace('below', 'above');
    }
    if (clean.includes('low') && !clean.includes('high')) {
      clean = clean.replace('low', 'high');
    }
    if (clean.includes('breakout')) {
      clean = clean.replace('breakout', 'breakdown');
    }
  } else {
    if (clean.includes('above')) {
      clean = clean.replace('above', 'below');
    }
    if (clean.includes('high') && !clean.includes('low')) {
      clean = clean.replace('high', 'low');
    }
    if (clean.includes('breakdown')) {
      clean = clean.replace('breakdown', 'breakout');
    }
  }

  const matched = ALLOWED_STOP_REASONS.get(clean);
...
function formatVolumeConfirmationText(volumeConfirmation = {}) {
  ...
  if (ratio === null) {
    if (volumeConfirmation && typeof volumeConfirmation === 'object' && volumeConfirmation.text) {
      return volumeConfirmation.text;
    }
    return '1.00x avg';
  }
...
export function formatManagedSignalText(signal = {}) {
  ...
  const stopReason = normalizeSignalStopReason(signal.stopReason, signal.direction);
