...
export function formatManagedSignalText(signal = {}) {
  ...
  const volumeText = signal.volumeConfirmation?.text || null;
  ...
  if (!entryMeta || !volumeText || !stopReason || !Number.isFinite(entryWidth)) {
    return `NO_SIGNAL|${signal.timeframe || '15m'}|${signal.pair || `${signal.symbol || 'BTC'}/USDT`}|${new Date().toISOString()}|SIGNAL_METADATA_MISSING`;
  }
