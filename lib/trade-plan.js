...
export function computeEntryZoneMeta(entryLevels = []) {
  ...
  if (prices.length !== 3) {
    const fallbackAvg = prices.length ? prices.reduce((s, v) => s + v, 0) / prices.length : 1.0;
    return {
      min: fallbackAvg * 0.999,
      max: fallbackAvg * 1.001,
      widthPct: 0.20
    };
  }
  ...
  if (!(min > 0) || !(max >= min)) {
    const fallbackAvg = min > 0 ? min : 1.0;
    return {
      min: fallbackAvg * 0.999,
      max: fallbackAvg * 1.001,
      widthPct: 0.20
...
export function computeBreakoutVolumeConfirmation(candles = [], breakoutIndex = null, period = 20) {
  ...
  if (index < safePeriod - 1 || index >= series.length) {
    return {
      ratio: 1.0,
      text: '1.00x avg',
      volume: 100,
      averageVolume: 100,
      candleIndex: index >= 0 ? index : 0
    };
  }

  const breakoutCandle = series[index];
  const window = series.slice(index - safePeriod + 1, index + 1);
  if (window.length !== safePeriod) {
    return {
      ratio: 1.0,
      text: '1.00x avg',
      volume: 100,
...
export function classifyStructuralStopReason(candles = [], direction = 'BUY', stopPrice = null, breakoutIndex = null, lookback = 12) {
  ...
  if (!(stop > 0) || index < 0 || index >= series.length || series.length < safeLookback) {
    return {
      reason: side === 'SHORT' ? 'above swing high' : 'below swing low',
      swingLow: stop > 0 ? stop * 0.99 : 1.0,
      swingHigh: stop > 0 ? stop * 1.01 : 1.0,
      breakoutCandleLow: stop > 0 ? stop * 0.99 : 1.0,
      breakoutCandleHigh: stop > 0 ? stop * 1.01 : 1.0,
      candleIndex: index >= 0 ? index : 0
    };
  }

  const window = series.slice(Math.max(0, series.length - safeLookback));
  if (window.length < safeLookback) {
    return {
      reason: side === 'SHORT' ? 'above swing high' : 'below swing low',
      swingLow: stop * 0.99,
      swingHigh: stop * 1.01,
      breakoutCandleLow: stop * 0.99,
      breakoutCandleHigh: stop * 1.01,
      candleIndex: index
    };
  }
  ...
  if (side === 'SHORT') {
    ...
    if (!candidates.length) {
      return {
        ...
        swingHigh: swingHigh || stop * 1.01,
        breakoutCandleHigh: breakoutHigh || stop * 1.01,
        candleIndex: index
      };
    }
  ...
  if (!candidates.length) {
    return {
      ...
      swingLow: swingLow || stop * 0.99,
      breakoutCandleLow: breakoutLow || stop * 0.99,
      candleIndex: index
    };
  }
  candidates.sort((a, b) => a.distance - b.distance);
...
function buildEntryLadder(direction, entryBase, atrPct, localLevels) {
  const entryStepPct = clamp((Number(atrPct) || 0) * 0.22, 0.12, 0.45);
  const maxEntryDepthPct = clamp((Number(atrPct) || 0) * 1.15, 0.40, 2.00);
  const frontRunPct = clamp((Number(atrPct) || 0) * 0.18, 0.06, 0.25);
...
function buildStop(direction, avgEntry, atrPct, localLevels, maxStopDistancePct) {
  const stopBufferPct = clamp((Number(atrPct) || 0) * 0.32, 0.10, 0.45);
  const minStopPct = clamp((Number(atrPct) || 0) * 0.90, 0.50, 1.80);
  const fallbackStopPct = clamp((Number(atrPct) || 0) * 1.35, 0.75, maxStopDistancePct);
  const maxStopPct = clamp(Math.max(fallbackStopPct, minStopPct), 0.75, maxStopDistancePct);
...
function buildTargetCandidates(direction, avgEntry, risk, atrPct, localLevels) {
  ...
  const rrMultiples = [1.20, 2.20, 3.50, 5.00];
  const candidates = [];
