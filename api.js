...
export async function fetchChartData(symbol = 'BTC', interval = '1h', limit = 48) {
  try {
    ...
    markApiOk('Binance Klines', `${symbol} ${interval} (${candles.length})`);
    return candles;
  } catch (e) {
...
// Helper: Compute EMA Series (returning array of same length)
function computeEMASeries(data = [], period = 9) {
...
// Helper: Compute MACD from series
...
export async function fetchBinanceSpread(symbol = 'BTC') {
  try {
    const ticker = symbol.replace('USDT', '').toUpperCase();
    const res = await fetch(`https://api.binance.com/api/v3/ticker/bookTicker?symbol=${ticker}USDT`);
    if (!res.ok) return 0.001; // default 0.1%
    const data = await res.json();
    const bid = parseFloat(data.bidPrice);
    const ask = parseFloat(data.askPrice);
    if (bid > 0 && ask > 0) return (ask - bid) / ask;
    return 0.001;
  } catch {
    return 0.001;
  }
...
function calculateTechnicalScore(closes, patterns = [], direction = 'BUY') {
  const rsi = computeRSI(closes, 14);
  const rsiScore = Math.min(100, Math.max(0, 100 - Math.abs((rsi || 50) - 50) * 2));
  
  const macdData = computeMACDSeries(closes);
  let macdScore = 50;
  if (macdData) {
    const hist = macdData.histogram;
    const currentHist = hist[hist.length - 1];
    const prevHist = hist[hist.length - 2];
    const hist20 = hist.slice(-20);
    const minHist = Math.min(...hist20);
    const maxHist = Math.max(...hist20);
    
    const strength = maxHist > minHist ? ((currentHist - minHist) / (maxHist - minHist)) * 100 : 50;
    const dirScore = direction === 'BUY' 
      ? (currentHist > prevHist ? 100 : (currentHist < prevHist ? 0 : 50))
      : (currentHist < prevHist ? 100 : (currentHist > prevHist ? 0 : 50));
      
    // Simplified recency
    let recency = 100;
    for (let i = 1; i < 10; i++) {
      if ((hist[hist.length-i] > 0 && hist[hist.length-i-1] <= 0) || (hist[hist.length-i] < 0 && hist[hist.length-i-1] >= 0)) {
        recency = Math.max(0, 100 - i * 10);
        break;
      }
    }
    
    // Divergence Bonus (Heuristic)
    let divergenceBonus = 0;
    if (closes.length > 20) {
      const pLast = closes[closes.length - 1];
      const pPrev = closes[closes.length - 10];
      const mLast = currentHist;
      const mPrev = hist[hist.length - 10];
      if (direction === 'BUY' && pLast < pPrev && mLast > mPrev) divergenceBonus = 10;
      if (direction === 'SELL' && pLast > pPrev && mLast < mPrev) divergenceBonus = 10;
    }
    
    macdScore = Math.min(100, (strength * 0.4) + (dirScore * 0.3) + (recency * 0.3) + divergenceBonus);
  }

  let patternScore = 50;
  if (patterns && patterns.length > 0) {
    const highReliability = ['flag', 'engulfing', 'cup', 'shoulders'];
    const hasHigh = patterns.some(p => highReliability.some(hr => p.name.toLowerCase().includes(hr)));
    patternScore = hasHigh ? 120 : 100;
  }
  patternScore = Math.min(100, patternScore);
  
  return (rsiScore * 0.3) + (macdScore * 0.4) + (patternScore * 0.3);
}

function calculateWhaleScore(netOutflow = 0) {
  // exchange net outflow > $500k -> 100; net inflow > $500k -> 0
  if (netOutflow >= 500000) return 100;
  if (netOutflow <= -500000) return 0;
  return ((netOutflow + 500000) / 1000000) * 100;
}

function calculateEMAConfluenceScore(closes, direction = 'BUY') {
  const ema9Series = computeEMASeries(closes, 9);
  const ema21Series = computeEMASeries(closes, 21);
  const curPrice = closes[closes.length - 1];
  const curEma9 = ema9Series[ema9Series.length - 1];
  const curEma21 = ema21Series[ema21Series.length - 1];
  const prevEma9 = ema9Series[ema9Series.length - 2];
  const prevEma21 = ema21Series[ema21Series.length - 2];

  if (direction === 'BUY') {
    if (curPrice > curEma9 && curPrice > curEma21 && curEma9 > prevEma9 && curEma21 > prevEma21) return 100;
    if ((curPrice > curEma9 && curPrice < curEma21) || (curPrice < curEma9 && curPrice > curEma21)) return 50;
    if (curPrice < curEma9 && curPrice < curEma21) return 0;
  } else {
    if (curPrice < curEma9 && curPrice < curEma21 && curEma9 < prevEma9 && curEma21 < prevEma21) return 100;
    if ((curPrice > curEma9 && curPrice < curEma21) || (curPrice < curEma9 && curPrice > curEma21)) return 50;
    if (curPrice > curEma9 && curPrice > curEma21) return 0;
...
function calculateVolumeScore(volumes, period = 5) {
...
export async function evaluateNexusSignal(symbol, timeframe = 'SCALP') {
  const interval = timeframe === 'SCALP' ? '1m' : '15m';
  const limit = 40;
  const candles = await fetchChartData(symbol, interval, limit);
  if (!candles) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|DATA_FETCH_FAIL`;
  ...
  let emaCross = false;
  const lookback = timeframe === 'SCALP' ? 3 : 5;
  // BUY: EMA9 crossed above EMA21
  // SELL: EMA9 crossed below EMA21
  const checkBuyCross = () => {
    for (let i = 0; i < lookback; i++) {
      const idx = ema9.length - 1 - i;
      if (ema9[idx] > ema21[idx] && ema9[idx-1] <= ema21[idx-1]) return true;
    }
    return false;
  };
  const checkSellCross = () => {
    for (let i = 0; i < lookback; i++) {
      const idx = ema9.length - 1 - i;
      if (ema9[idx] < ema21[idx] && ema9[idx-1] >= ema21[idx-1]) return true;
    }
  ...
  const isBuy = checkBuyCross();
  const isSell = checkSellCross();
  const direction = isBuy ? 'BUY' : (isSell ? 'SELL' : null);
  
  if (!direction) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|EMA_CROSS_FAIL`;

  const volAvgPeriod = timeframe === 'SCALP' ? 3 : 5;
  const volThreshold = timeframe === 'SCALP' ? 0.8 : 0.7;
  const curVol = volumes[volumes.length - 1];
  const avgVol = volumes.slice(-volAvgPeriod-1, -1).reduce((s,v) => s+v, 0) / volAvgPeriod;
  if (curVol <= avgVol * volThreshold) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|VOLUME_FAIL`;

  const spread = await fetchBinanceSpread(symbol);
  if (spread >= 0.0015) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|SPREAD_FAIL`;

  // 2. 7 Pillars
  const candlePatterns = await fetchCandlePatterns(symbol, interval);
  const patterns = candlePatterns?.patterns || [];
  
  const techScore = calculateTechnicalScore(closes, patterns, direction);
  const whaleScore = 50; // Placeholder
  const emaScore = calculateEMAConfluenceScore(closes, direction);
  const volScore = calculateVolumeScore(volumes, timeframe === 'SCALP' ? 3 : 5);
  const sentScore = 50; // Placeholder
  const newsScore = 50; // Placeholder
  const alphaScoreSources = 50; // Placeholder

  // 3. Adaptive Weighting
  const isTrending = sentScore > 65 || sentScore < 35;
  let weights = isTrending 
    ? { tech: 0.22, whale: 0.20, ema: 0.15, vol: 0.08, sent: 0.15, news: 0.12, alpha: 0.10 }
    : { tech: 0.15, whale: 0.15, ema: 0.08, vol: 0.15, sent: 0.25, news: 0.15, alpha: 0.12 };

  const rawAlpha = (techScore * weights.tech) + (whaleScore * weights.whale) + (emaScore * weights.ema) + 
                   (volScore * weights.vol) + (sentScore * weights.sent) + (newsScore * weights.news) + 
                   (alphaScoreSources * weights.alpha);
  const alpha = Math.min(100, Math.max(0, Math.round(rawAlpha)));

  // 4. Benchmarks
  const alphaThreshold = timeframe === 'SCALP' ? 65 : 70;
  if (alpha < alphaThreshold) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|ALPHA_LOW:${alpha}`;

  if (techScore < 45) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|PILLAR_FLOOR_FAIL:Technical`;
  const emaFloor = isTrending ? 50 : 40;
  if (emaScore < emaFloor) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|PILLAR_FLOOR_FAIL:EMA`;
  const volFloor = timeframe === 'SCALP' ? 40 : 35;
  if (volScore < volFloor) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|PILLAR_FLOOR_FAIL:Volume`;

  const poorPillars = [techScore, whaleScore, emaScore, volScore, sentScore, newsScore, alphaScoreSources].filter(s => s < 40).length;
  if (poorPillars >= 3) return `NO_SIGNAL|${symbol}|${new Date().toISOString()}|TOO_MANY_POOR_PILLARS`;

  // 5. Price Levels
  const isAlt = !['BTC', 'ETH'].includes(symbol.toUpperCase());
  let tp1, tp2, sl;
  if (timeframe === 'SCALP') {
    if (!isAlt) {
      tp1 = isBuy ? curPrice * 1.0025 : curPrice * 0.9975;
      tp2 = isBuy ? curPrice * 1.0040 : curPrice * 0.9960;
      sl = isBuy ? curPrice * 0.9985 : curPrice * 1.0015;
    } else {
      tp1 = isBuy ? curPrice * 1.0035 : curPrice * 0.9965;
      tp2 = isBuy ? curPrice * 1.0050 : curPrice * 0.9950;
      sl = isBuy ? curPrice * 0.9980 : curPrice * 1.0020;
    }
  } else {
    if (!isAlt) {
      tp1 = isBuy ? curPrice * 1.010 : curPrice * 0.990;
      tp2 = isBuy ? curPrice * 1.018 : curPrice * 0.982;
      sl = isBuy ? curPrice * 0.994 : curPrice * 1.006;
    } else {
      tp1 = isBuy ? curPrice * 1.015 : curPrice * 0.985;
      tp2 = isBuy ? curPrice * 1.025 : curPrice * 0.975;
      sl = isBuy ? curPrice * 0.992 : curPrice * 1.008;
    }
  }

  const fmt = (p) => p < 1 ? p.toFixed(5) : p < 100 ? p.toFixed(3) : p.toFixed(2);
  
  return `SIGNAL|${timeframe}|${symbol}/USDT|${direction}|${fmt(curPrice)}|${fmt(tp1)}|${fmt(tp2)}|${fmt(sl)}|NONE|${new Date().toISOString()}|${alpha}`;
}
